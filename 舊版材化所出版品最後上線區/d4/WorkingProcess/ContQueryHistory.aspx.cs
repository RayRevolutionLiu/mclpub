...
{
	/// <summary>
	/// Summary description for ContQueryHistory.
	/// </summary>
	public class ContQueryHistory : MRLPub.d4.Pages.Page
	{
		protected System.Web.UI.WebControls.LinkButton lnbBack;
		protected System.Web.UI.WebControls.DataGrid dgdCont;
		protected System.Web.UI.WebControls.Panel pnlNoHistory;
		protected System.Web.UI.WebControls.Label lblNoHistory;
		protected System.Web.UI.WebControls.Button btnNewCont;
		...
		private void Page_Load(object sender, System.EventArgs e)
		{
			...
			if (!Page.IsPostBack)
			{
				...
				if (DataCount>0)
				{
					...
					lblCustFound.Text = "此客戶共有 " + DataCount.ToString() + " 筆歷史資料";
		...
		{
			Response.Redirect("~/d4/WorkingProcess/ContQueryCust.aspx");
		}
		#endregion
		...
		private int Bind_dgdCont()
		{
			...
			DataSet ds = cont.GetContractsByCustNo(custno);
		...
		{
			string old_contno = dgdCont.SelectedItem.Cells[0].Text.Trim();
			Response.Redirect("~/d4/WorkingProcess/ContNew.aspx?old_contno=" + old_contno);
		}
		#endregion
		...
		#region 無歷史合約...直接新增
		private void btnNewCont_Click(object sender, System.EventArgs e)
		{
			string custno;
			if (Request.QueryString["custno"] == null || Request.QueryString["custno"] == "")
			{
			...
			Response.Redirect("~/d4/WorkingProcess/ContNew.aspx?CustNo=" + custno);		
		}
		#endregion
	}
}

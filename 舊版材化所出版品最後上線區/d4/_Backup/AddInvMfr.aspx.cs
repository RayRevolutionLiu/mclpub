	...
	public class AddInvMfr : System.Web.UI.Page
	{
		...
		private void Page_Load(object sender, System.EventArgs e)
		{
			...
			if (!Page.IsPostBack)
			{
				...
				if(Request.QueryString["OldContNo"].ToString().Trim() !="" || Request.QueryString["OldContNo"].ToString().Trim() != "0")
				{
					BindGrid1();
				}
				else
		...
		{    
			...
			// sqlSelectCommand1
			// 
			this.sqlSelectCommand1.CommandText = "SELECT cont_contid, cont_syscd, cont_contno, cont_mfrno, cont_custno FROM dbo.c4_" +
				"cont";
			this.sqlSelectCommand1.Connection = this.sqlCnnISCCOM1;
			// 
			// sqlSelectCommand5
			// 
			this.sqlSelectCommand5.CommandText = "SELECT MAX(im_imseq) AS MaxIMSeq, im_contno FROM dbo.invmfr WHERE (im_syscd = \'C4" +
				"\') GROUP BY im_contno";
			...
			// sqlCommand3
			// 
			this.sqlCommand3.CommandText = @"UPDATE dbo.invmfr SET im_mfrno = @mfrno, im_nm = @nm, im_jbti = @jbti, im_zip = @zip, im_addr = @addr, im_tel = @tel, im_fax = @fax, im_cell = @cell, im_email = @email, im_invcd = @invcd, im_taxtp = @taxtp, im_fgitri = @fgitri WHERE (im_syscd = @syscd) AND (im_contno = @contno) AND (im_imseq = @imseq)";
			this.sqlCommand3.Connection = this.sqlCnnISCCOM1;
			...
			// sqlCommand2
			// 
			this.sqlCommand2.CommandText = "DELETE FROM dbo.invmfr WHERE (im_syscd = @syscd) AND (im_contno = @contno) AND (i" +
				"m_imseq = @imseq)";
			this.sqlCommand2.Connection = this.sqlCnnISCCOM1;
			...
			// sqlCommand1
			// 
			this.sqlCommand1.CommandText = @"INSERT INTO dbo.invmfr (im_syscd, im_contno, im_imseq, im_mfrno, im_nm, im_jbti, im_zip, im_addr, im_tel, im_fax, im_cell, im_email, im_invcd, im_taxtp, im_fgitri) VALUES (@syscd, @contno, @imseq, @mfrno, @nm, @jbti, @zip, @addr, @tel, @fax, @cell, @email, @invcd, @taxtp, @fgitri)";
			this.sqlCommand1.Connection = this.sqlCnnISCCOM1;
		...
		private void InitialData()
		{
			...
			// 載入預設資料 - 合約書編號
			if(Context.Request.QueryString["NewContNo"].ToString().Trim() != "" || Context.Request.QueryString["NewContNo"].ToString().Trim() != null)
			{
				contno = Context.Request.QueryString["NewContNo"].ToString().Trim();
				//Response.Write("contno= " + contno + "<BR>");
			}
			
			// 載入預設資料 - 抓出廠商編號及客戶資料
			if(Request.QueryString["CustNo"].ToString().Trim() != "" || Request.QueryString["CustNo"].ToString().Trim() != null)
			{
...
//				Response.Write("dv2.RowFilter= " + dv2.RowFilter + "<BR>");
		...
		private void BindGrid1()
		{
			...
			if(Request.QueryString["OldContNo"].ToString().Trim() != "" || Request.QueryString["OldContNo"].ToString().Trim() != "0" )
			{
				old_contno = Request.QueryString["OldContNo"].ToString().Trim();
		...
		private void BindGrid2()
		{
			...
			string contno = "";
			if(Request.QueryString["NewContNo"].ToString().Trim() != "")
			{
				contno = Context.Request.QueryString["NewContNo"].ToString().Trim();
		...
		private void CheckDuplication()
		{
			// 抓出 畫面上的值
			string strSyscd = "C4";
			string strContNo = Context.Request.QueryString["NewContNo"].ToString().Trim();
		...
		private void InsertDB()
		{
			...
			string strIMtaxtp = "1";
			if (!(this.ddlIMTaxtp.SelectedIndex < 0))
				strIMtaxtp = this.ddlIMTaxtp.SelectedItem.Value.ToString().Trim();
		...
		private void DataGrid1_ItemCommand(object source, System.Web.UI.WebControls.DataGridCommandEventArgs e)
		{
			...
			string strsyscd = "C4";
			string strcontno = Request.QueryString["OldContNo"].ToString().Trim();
			...
			if (dv4.Count > 0)
			{
				...
				this.ddlIMInvtp.Items.FindByValue(dv4[0]["im_invcd"].ToString().Trim()).Selected = true;
				this.ddlIMTaxtp.Items.FindByValue(dv4[0]["im_taxtp"].ToString().Trim()).Selected = true;
				if (dv4[0]["im_fgitri"].ToString().Trim() == "")
					this.ddlIMfgITRI.SelectedIndex = 0;
				else
					this.ddlIMfgITRI.Items.FindByValue(dv4[0]["im_fgitri"].ToString().Trim()).Selected = true;
			}

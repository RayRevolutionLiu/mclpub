...
namespace mclpub.Contract
{
    public partial class PlaneCont2Edit : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!IsPostBack)
            {
                try
                {
                    ...
                    InitialData();
                    string StRcont = sec.decryptquerystring(Context.Request.QueryString["cont_contno"].ToString().Trim());
                    if (StRcont != "0")
                    {
                        this.lblContNo.Text = StRcont;
        ...
        private void GetMfrCust()
        {
            ...
            string custno = "";
            string StRcust = sec.decryptquerystring(Context.Request.QueryString["cust_custno"].ToString().Trim());
            // 若有客戶編號, 則指定顯示之
            if (StRcust != "")
            {
                custno = StRcust;
        ...
        {
            // 抓出網頁變數: 客戶編號
            string strContNo = sec.decryptquerystring(Context.Request.QueryString["cont_contno"].ToString().Trim());
        ...
        protected void btnGoPub_Click(object sender, EventArgs e)
        {
            ...
            strbuf = "../Layout/PubFm.aspx?contno=" + strContNo + "&bkcd=" + strBkcd + "&fgnew=1";

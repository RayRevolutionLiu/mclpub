...
<asp:Content id="Content1" ContentPlaceholderID="ContentPlaceHolder1" runat="server">
    ...
    <span class="font_size12 font_darkblue font_bold">目前位置:&nbsp;訂戶管理 / 雜誌叢書訂單處理 / <asp:Label ID="lblTitle" runat="server"></asp:Label> / 步驟三:新增訂單</span> 
    <span class="stripeMe">
    <br />
				...
				<table dataFld="訂單" id="Maintable" dataSrc="#DSO0" cellSpacing="0" cellPadding="0" width="100%" class="font_blacklink font_size13">
					...
					<tr>
						...
						<td style="WIDth: 192px">
                            ...
                            <input id="EmptyOrder" type="hidden" runat="server" />
...
xmlDoc0.loadXML(document.getElementById("<% =EmptyOrder.ClientID %>").value);
...
//xmlReceivers.getElementsByTagName("公司名稱")[0].childNodes[0].nodeValue
//xmlReceivers.getElementsByTagName("地址")[0].childNodes[0].nodeValue
//xmlReceivers.getElementsByTagName("姓名")[0].childNodes[0].nodeValue
//xmlReceivers.getElementsByTagName("職稱")[0].childNodes[0].nodeValue
//xmlReceivers.getElementsByTagName("郵遞區號")[0].childNodes[0].nodeValue
//xmlReceivers.getElementsByTagName("電話")[0].childNodes[0].nodeValue
//xmlReceivers.getElementsByTagName("序號")[0].childNodes[0].nodeValue
xmlReceivers.childNodes.item(0).selectSingleNode("公司名稱").text = document.getElementById("<% =hiddenCoName.ClientID%>").value;
xmlReceivers.childNodes.item(0).selectSingleNode("地址").text = document.getElementById("<% =tbxAddress.ClientID%>").value;
xmlReceivers.childNodes.item(0).selectSingleNode("姓名").text = document.getElementById("<% =tbxInvoiceName.ClientID%>").value;
xmlReceivers.childNodes.item(0).selectSingleNode("職稱").text = document.getElementById("<% =tbxInvoiceJob.ClientID%>").value;
xmlReceivers.childNodes.item(0).selectSingleNode("郵遞區號").text = document.getElementById("<% =tbxPostCode.ClientID%>").value;
xmlReceivers.childNodes.item(0).selectSingleNode("電話").text = document.getElementById("<% =tbxTel.ClientID%>").value;
...
//alert(xmlItems.xml);
if ((document.getElementById("<% =tbxInvoiceJob.ClientID%>").value == "小姐") || (document.getElementById("<% =tbxInvoiceJob.ClientID%>").value == "先生"))
    xmlHeader.selectSingleNode("發票收件人職稱").text = window.document.getElementById("<% =tbxInvoiceJob.ClientID%>").value;
else
...
function doChange(){
	if(xmlHeader.selectSingleNode("發票收件人職稱").text=="其他")
	    document.getElementById("<% =tbxInvoiceJob.ClientID%>").value = "";
	else
	    document.getElementById("<% =tbxInvoiceJob.ClientID%>").value = xmlHeader.selectSingleNode("發票收件人職稱").text;
}
...
function doDelete(obj) {
    ...
    var idx = 0;
    for (idx = 0; idx < parentNodetrType.length; idx++) {
        if (parentNodetrType[idx].getAttribute('custom') == 'del') {
            break;
        }
    }
    idx = idx - 2;  //因為parentNodetrType會連表頭的tr都抓到 所以要減二 因為他有兩個表頭
...
function doGetrec()
{
    ...
    if (document.getElementById("<% =hiddenRec.ClientID%>").value == "") {
		myObject.flag=false;
		alert("沒有歷史資料, 請自行輸入資料");
	}
	else{
	    myObject.flag=true;
	    myObject.prexmldata = document.getElementById("<% =hiddenRec.ClientID%>").value;
	}
    myObject.xmldata=xmlReceivers.xml;
    myObject.CoName = document.getElementById("<% =hiddenCoName.ClientID%>").value;
    myObject.Name = document.getElementById("<% =tbxInvoiceName.ClientID%>").value;
    myObject.Address = document.getElementById("<% =tbxAddress.ClientID%>").value;
    myObject.PostCode = document.getElementById("<% =tbxPostCode.ClientID%>").value;
    myObject.Job = document.getElementById("<% =tbxInvoiceJob.ClientID%>").value;
    myObject.Tel = document.getElementById("<% =tbxTel.ClientID%>").value;
    vreturn=window.showModalDialog("RecForm.aspx", myObject, "dialogHeight:400px;dialogWidth:1000px;center:yes;scroll:yes;status:no;help:no;"); 
...
function pick_date(theField){
	...
	strFeature = "";
	strFeature += "dialogHeight:350px;dialogWidth:380px;center:yes;scroll:yes;status:no;help:no;";
...
function SaveProc() {
    ...
    xmlHeader.selectSingleNode("訂戶編號").text = window.document.getElementById("<% =hiddenID.ClientID%>").value;
    xmlHeader.selectSingleNode("訂單流水號").text = window.document.getElementById("<% =hiddenOrderNo.ClientID%>").value;
    xmlHeader.selectSingleNode("訂購類別一").text = window.document.getElementById("<% =hiddenType1.ClientID%>").value;
    if (window.document.getElementById("<% =hiddenType1.ClientID%>").value == "09")
        xmlHeader.selectSingleNode("訂購類別二").text = "00";
    else
        xmlHeader.selectSingleNode("訂購類別二").text = window.document.getElementById("<% =ddlOrderType2.ClientID%>").value;
    xmlHeader.selectSingleNode("統一編號").text = window.document.getElementById("<% =tbxMfrno.ClientID%>").value;
    if (window.document.getElementById("<% =tbxInvoiceName.ClientID%>").value == "") {
        alert("發票收件人姓名不可空白");
        document.getElementById("<% =tbxInvoiceName.ClientID%>").focus();
        return;
    }
    if (window.document.getElementById("<% =tbxTel.ClientID%>").value == "") {
        alert("發票收件人電話不可空白");
        document.getElementById("<% =tbxTel.ClientID%>").focus();
        return;
    }
    if (window.document.getElementById("<% =tbxAddress.ClientID%>").value == "") {
        alert("發票收件人地址不可空白");
        document.getElementById("<% =tbxAddress.ClientID%>").focus();
        return;
    }
    if (window.document.getElementById("<% =tbxPostCode.ClientID%>").value == "") {
        alert("發票收件人郵遞區號不可空白");
        document.getElementById("<% =tbxPostCode.ClientID%>").focus();
        return;
    }
    if (window.document.getElementById("<% =ddlSpn.ClientID%>").value == "") {
        alert("請選擇承辦業務員");
        document.getElementById("<% =ddlSpn.ClientID%>").focus();


        return;
    }
    //20121123 新增判斷訂單明細都有要收件人 否則對應不上
    var num = xmlItems.getElementsByTagName("序號");
    ...
    xmlHeader.selectSingleNode("發票收件人姓名").text = window.document.getElementById("<% =tbxInvoiceName.ClientID%>").value;
    if (xmlHeader.selectSingleNode("發票收件人職稱").text == "其他")
        xmlHeader.selectSingleNode("發票收件人職稱").text = window.document.getElementById("<% =tbxInvoiceJob.ClientID%>").value;
    xmlHeader.selectSingleNode("發票收件人地址").text = window.document.getElementById("<% =tbxAddress.ClientID%>").value;
    xmlHeader.selectSingleNode("發票收件人郵遞區號").text = window.document.getElementById("<% =tbxPostCode.ClientID%>").value;
    xmlHeader.selectSingleNode("發票收件人電話").text = window.document.getElementById("<% =tbxTel.ClientID%>").value;
    xmlHeader.selectSingleNode("發票收件人傳真").text = window.document.getElementById("<% =tbxFax.ClientID%>").value;
    xmlHeader.selectSingleNode("發票收件人手機").text = window.document.getElementById("<% =tbxCell.ClientID%>").value;
    xmlHeader.selectSingleNode("發票收件人Email").text = window.document.getElementById("<% =tbxEmail.ClientID%>").value;
    xmlHeader.selectSingleNode("訂購日期").text = window.document.getElementById("<% =tbxOrderDate.ClientID%>").value;
    var myObject = new Object();
    strbuf = "C1" + document.getElementById("<% =hiddenID.ClientID%>").value + document.getElementById("<% =hiddenType1.ClientID%>").value + document.getElementById("<% =hiddenOrderNo.ClientID%>").value;
    xmlHeader.selectSingleNode("訂戶編號").text = window.document.getElementById("<% =hiddenID.ClientID%>").value;
    xmlHeader.selectSingleNode("訂單流水號").text = window.document.getElementById("<% =hiddenOrderNo.ClientID%>").value;
    xmlHeader.selectSingleNode("訂購類別一").text = window.document.getElementById("<% =hiddenType1.ClientID%>").value;
...
//    }
//    else if (xmlHTTP.statusText == "Max_Length") {
//        alert("訂單明細資料過多,請分批新增");
//    }
//    else {
//        alert(xmlHTTP.responseText);
//    }

...
<HTML>
	...
	<body>
		<form id="RptBookMntUnPub" method="post" runat="server">
			<%
			'-----------------------------------------------------------------------------
			...
			sqlCmdData = SqlCmdData & "dbo.wk_c4_mail_mnt.mtpcd, dbo.wk_c4_mail_mnt.unpubmnt, dbo.mtp.mtp_nm "
			...
			sqlCmdData = SqlCmdData & "WHERE (dbo.wk_c4_mail_mnt.unpubmnt > 0) "
			...
			sqlCmdData = SqlCmdData & "dbo.wk_c4_mail_mnt.unpubmnt, dbo.mtp.mtp_nm "
			sqlCmdData = SqlCmdData & "ORDER BY  dbo.wk_c4_mail_mnt.mtpcd, dbo.wk_c4_mail_mnt.unpubmnt "
			...
			DIM sum_unpubmnt, count_unpubmnt
			DIM preMtp
			' -- 顯示資料 --
			IF (rsData.EOF) THEN
			    ' 無資料就給訊息
...
'						IF (j<>0) THEN
							...
							A1(i, 2) = ""
							A1(i, 3) = "合計："
							A1(i, 4) = CStr(count_unpubmnt) & "份"
							A1(i, 5) = CStr(sum_unpubmnt) & "本"
...
'						END IF
					...
					A1(i, 3) = rsData("unpubmnt").Value & "x"
					A1(i, 4) = rsData("pub_count").Value & "份="
					count_unpubmnt = count_unpubmnt + rsData("pub_count").Value
					A1(i, 5) = rsData("unpubmnt").Value * rsData("pub_count").Value & "本"
					sum_unpubmnt = sum_unpubmnt + rsData("unpubmnt").Value * rsData("pub_count").Value
...
'						DIM highlight1, highlight2, highlight3, highlight4, highlight5, highlight6, highlight7, highlight8
				...
				' 範本Template
				SrcBook = Server.MapPath("RptBookMntUnPub.xls")
				
				' Bind資料到Template中
				XLS.Generate(SrcBook, "RptBookMntUnPub.xls", TRUE)

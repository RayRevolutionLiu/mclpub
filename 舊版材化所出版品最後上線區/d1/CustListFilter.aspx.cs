...
{
	/// <summary>
	/// Summary description for CustListFilter.
	/// </summary>
	public class CustListFilter : System.Web.UI.Page
	{
		...
		private void Page_Load(object sender, System.EventArgs e)
		{
			...
			if (!IsPostBack)
			{
				...
				dv2.RowFilter="otp_otp1cd='"+ddlOrderType1.SelectedItem.Value+"'";
				...
				dv3.RowFilter="obtp_otp1cd='"+ddlOrderType1.SelectedItem.Value+"'";
				...
				if(ddlMosea.SelectedItem.Value=="0")
					dv4.RowFilter="mtp_mtpcd LIKE '1%'";
				else
					dv4.RowFilter="mtp_mtpcd LIKE '2%'";
				ddlMailType.DataSource=dv4;
		...
		{    
			...
			this.sqlSelectCommand1.CommandText = "SELECT dbo.c1_od.od_odid, dbo.c1_od.od_syscd, dbo.c1_od.od_custno, dbo.c1_od.od_o" +
				...
				"bo.c1_od.od_otp1cd + dbo.c1_od.od_otp1seq AS nostr, dbo.c1_order.o_date, dbo.c1_" +
				"obtp.obtp_obtpnm, dbo.c1_obtp.obtp_obtpcd, dbo.c1_obtp.obtp_otp1cd, dbo.c1_od.od" +
				"_sdate + \'~\' + dbo.c1_od.od_edate AS datestr, dbo.c1_order.o_fgpreinv, dbo.c1_or" +
				"der.o_indate, dbo.c1_order.o_empno, dbo.c1_ramt.ra_mtpcd, dbo.c1_order.o_otp2cd " +
				"FROM dbo.c1_od INNER JOIN dbo.c1_order ON dbo.c1_od.od_syscd = dbo.c1_order.o_sy" +
				"scd AND dbo.c1_od.od_custno = dbo.c1_order.o_custno AND dbo.c1_od.od_otp1cd = db" +
				"o.c1_order.o_otp1cd AND dbo.c1_od.od_otp1seq = dbo.c1_order.o_otp1seq INNER JOIN" +
				" dbo.c1_ramt ON dbo.c1_od.od_syscd = dbo.c1_ramt.ra_syscd AND dbo.c1_od.od_custn" +
				"o = dbo.c1_ramt.ra_custno AND dbo.c1_od.od_otp1cd = dbo.c1_ramt.ra_otp1cd AND db" +
				"o.c1_od.od_otp1seq = dbo.c1_ramt.ra_otp1seq AND dbo.c1_od.od_oditem = dbo.c1_ram" +
				"t.ra_oditem INNER JOIN dbo.c1_or ON dbo.c1_ramt.ra_syscd = dbo.c1_or.or_syscd AN" +
				"D dbo.c1_ramt.ra_custno = dbo.c1_or.or_custno AND dbo.c1_ramt.ra_otp1cd = dbo.c1" +
				"_or.or_otp1cd AND dbo.c1_ramt.ra_otp1seq = dbo.c1_or.or_otp1seq AND dbo.c1_ramt." +
				"ra_oritem = dbo.c1_or.or_oritem LEFT OUTER JOIN dbo.c1_obtp ON dbo.c1_od.od_btpc" +
				"d = dbo.c1_obtp.obtp_obtpcd AND dbo.c1_od.od_otp1cd = dbo.c1_obtp.obtp_otp1cd WH" +
				"ERE (dbo.c1_od.od_syscd = \'C1\')";
			...
			// sqlSelectCommand5
			// 
			this.sqlSelectCommand5.CommandText = "SELECT id, tmp_no, tmp_pytpnm, tmp_fgpreinv, tmp_obtpcd, tmp_obtpnm, tmp_datestr," +
				" tmp_amt, tmp_nm, tmp_inm, tmp_zip, tmp_addr, tmp_tel, tmp_mnt, tmp_rmcont, tmp_" +
				"rmdate, tmp_cname, tmp_pyno FROM dbo.tmp_list1";
			this.sqlSelectCommand5.Connection = this.sqlConnection1;
			// 
			// sqlSelectCommand4
			// 
			this.sqlSelectCommand4.CommandText = "SELECT mtp_mtpid, mtp_mtpcd, mtp_nm FROM dbo.mtp";
			...
			// sqlDataAdapter6
			// 
			this.sqlDataAdapter6.DeleteCommand = this.sqlDeleteCommand2;
			this.sqlDataAdapter6.InsertCommand = this.sqlInsertCommand2;
			...
			this.sqlDataAdapter6.UpdateCommand = this.sqlUpdateCommand2;
			// 
			// sqlDeleteCommand2
			// 
			...
			// sqlDataAdapter5
			// 
			this.sqlDataAdapter5.DeleteCommand = this.sqlDeleteCommand1;
			this.sqlDataAdapter5.InsertCommand = this.sqlInsertCommand1;
			...
			this.sqlDataAdapter5.UpdateCommand = this.sqlUpdateCommand1;
			// 
			// sqlDeleteCommand1
			// 
			...
			// sqlInsertCommand1
			// 
			this.sqlInsertCommand1.CommandText = @"INSERT INTO dbo.tmp_list1(tmp_no, tmp_pytpnm, tmp_fgpreinv, tmp_obtpcd, tmp_obtpnm, tmp_datestr, tmp_amt, tmp_nm, tmp_inm, tmp_zip, tmp_addr, tmp_tel, tmp_mnt, tmp_rmcont, tmp_rmdate, tmp_cname, tmp_pyno) VALUES (@tmp_no, @tmp_pytpnm, @tmp_fgpreinv, @tmp_obtpcd, @tmp_obtpnm, @tmp_datestr, @tmp_amt, @tmp_nm, @tmp_inm, @tmp_zip, @tmp_addr, @tmp_tel, @tmp_mnt, @tmp_rmcont, @tmp_rmdate, @tmp_cname, @tmp_pyno); SELECT id, tmp_no, tmp_pytpnm, tmp_fgpreinv, tmp_obtpcd, tmp_obtpnm, tmp_datestr, tmp_amt, tmp_nm, tmp_inm, tmp_zip, tmp_addr, tmp_tel, tmp_mnt, tmp_rmcont, tmp_rmdate, tmp_cname, tmp_pyno FROM dbo.tmp_list1 WHERE (id = @@IDENTITY)";
			...
			this.sqlInsertCommand1.Parameters.Add(new System.Data.SqlClient.SqlParameter("@tmp_amt", System.Data.SqlDbType.Real, 4, System.Data.ParameterDirection.Input, false, ((System.Byte)(0)), ((System.Byte)(0)), "tmp_amt", System.Data.DataRowVersion.Current, null));
			...
			// sqlUpdateCommand1
			// 
			this.sqlUpdateCommand1.CommandText = @"UPDATE dbo.tmp_list1 SET tmp_no = @tmp_no, tmp_pytpnm = @tmp_pytpnm, tmp_fgpreinv = @tmp_fgpreinv, tmp_obtpcd = @tmp_obtpcd, tmp_obtpnm = @tmp_obtpnm, tmp_datestr = @tmp_datestr, tmp_amt = @tmp_amt, tmp_nm = @tmp_nm, tmp_inm = @tmp_inm, tmp_zip = @tmp_zip, tmp_addr = @tmp_addr, tmp_tel = @tmp_tel, tmp_mnt = @tmp_mnt, tmp_rmcont = @tmp_rmcont, tmp_rmdate = @tmp_rmdate, tmp_cname = @tmp_cname, tmp_pyno = @tmp_pyno WHERE (id = @Original_id); SELECT id, tmp_no, tmp_pytpnm, tmp_fgpreinv, tmp_obtpcd, tmp_obtpnm, tmp_datestr, tmp_amt, tmp_nm, tmp_inm, tmp_zip, tmp_addr, tmp_tel, tmp_mnt, tmp_rmcont, tmp_rmdate, tmp_cname, tmp_pyno FROM dbo.tmp_list1 WHERE (id = @Select_id)";
			...
			this.sqlUpdateCommand1.Parameters.Add(new System.Data.SqlClient.SqlParameter("@tmp_amt", System.Data.SqlDbType.Real, 4, System.Data.ParameterDirection.Input, false, ((System.Byte)(0)), ((System.Byte)(0)), "tmp_amt", System.Data.DataRowVersion.Current, null));
			...
			this.sqlUpdateCommand1.Parameters.Add(new System.Data.SqlClient.SqlParameter("@Select_id", System.Data.SqlDbType.Int, 4, System.Data.ParameterDirection.Input, false, ((System.Byte)(0)), ((System.Byte)(0)), "id", System.Data.DataRowVersion.Current, null));
			// 
			// sqlDataAdapter4
			// 
		...
		private void btnSearch_Click(object sender, System.EventArgs e)
		{
			...
			if(cbx1.Checked)
			{
				if(ddlCustType.SelectedItem.Value.Trim()=="新訂戶")
					strbuf+=" and od_otp1seq='001'";
				else
					strbuf+=" and od_otp1seq<>'001'";
			}
			if(cbx2.Checked)
			...
			if(cbx4.Checked)
			{
				strbuf+=" and od_otp1cd="+ddlOrderType1.SelectedItem.Value.Trim();
				strbuf+=" and o_otp2cd="+ddlOrderType2.SelectedItem.Value.Trim();
			}
			if(cbx5.Checked)
				strbuf+=" and od_btpcd="+ddlBookType.SelectedItem.Value.Trim();
			if(cbx6.Checked)
				strbuf+=" and ra_mtpcd ="+ddlMailType.SelectedItem.Value.Trim();
			if(cbx7.Checked)
				strbuf+=" and or_nm LIKE '%"+tbxRecName.Text.Trim()+"%'";
			if(cbx8.Checked)
			{
				date1=tbxDate.Text.Substring(0,4)+tbxDate.Text.Substring(5,2)+tbxDate.Text.Substring(8,2);
				strbuf+=" and (od_sdate <= '"+date1+"' and od_edate >= '"+date1+"')";
			}
			...
			else
			{
				...
				for(int i=0; i<dv1.Count; i++)
				{
					...
					this.sqlInsertCommand1.Parameters["@tmp_pytpnm"].Value="";
					this.sqlInsertCommand1.Parameters["@tmp_fgpreinv"].Value="";
					this.sqlInsertCommand1.Parameters["@tmp_obtpnm"].Value=dv1[i].Row["obtp_obtpnm"].ToString().Trim();
					this.sqlInsertCommand1.Parameters["@tmp_obtpcd"].Value=dv1[i].Row["ra_mtpcd"].ToString().Trim();
					...
					this.sqlInsertCommand1.Parameters["@tmp_rmcont"].Value="";
					this.sqlInsertCommand1.Parameters["@tmp_rmdate"].Value=dv1[i].Row["o_date"].ToString().Trim();
					this.sqlInsertCommand1.Parameters["@tmp_cname"].Value="";
					this.sqlInsertCommand1.Parameters["@tmp_pyno"].Value=dv1[i].Row["od_custno"].ToString().Trim();
		...
		private void btnPrintList_Click(object sender, System.EventArgs e)
		{
			...
			dv6.RowFilter="srspn_empno = '"+reg+"'";
			if(dv6.Count>=0)
			{
				string	cname=dv6[0].Row["srspn_cname"].ToString().Trim();
				string	strbuf="cust_list1.aspx?cname="+cname;

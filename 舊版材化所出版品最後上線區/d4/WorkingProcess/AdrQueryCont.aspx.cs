...
{
	/// <summary>
	/// Summary description for AdrQueryCont.
	/// </summary>
	public class AdrQueryCont : MRLPub.d4.Pages.Page
	{
		...
		private void lngGoThis_Click(object sender, System.EventArgs e)
		{
			...
			if (contno == "")
			{
				jsAlertMsg("BLANKCONTNO", "欲直接維護落版資料，合約編號不可空白");
				return;
			}
			...
			if (dr.Read())
			{
				Response.Redirect("AdrPublish.aspx?NewContNo=" + contno);
			}
			else
		...
		private void dgdCont_SelectedIndexChanged(object sender, System.EventArgs e)
		{
			...
			Response.Redirect("AdrPublish.aspx?NewContNo=" + NewContNo);
		}
		#endregion

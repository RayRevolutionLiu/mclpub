...
<HTML>
	...
	<body>
		<form id="RptRemailList" method="post" runat="server">
			<%
				'-----------------------------------------------------------------------------
				...
				sqlcmd1 = "SELECT c4_remail.*, c4_cont.cont_signdate, c4_cont.cont_sdate, c4_cont.cont_edate, c4_or.* "
				sqlcmd1 = sqlcmd1 & " FROM c4_remail INNER JOIN "
				sqlcmd1 = sqlcmd1 & " c4_cont ON c4_remail.rm_syscd = c4_cont.cont_syscd AND "
				sqlcmd1 = sqlcmd1 & " c4_remail.rm_contno = c4_cont.cont_contno INNER JOIN "
				sqlcmd1 = sqlcmd1 & " c4_or ON c4_remail.rm_syscd = c4_or.or_syscd AND "
				sqlcmd1 = sqlcmd1 & " c4_remail.rm_contno = c4_or.or_contno AND "
				sqlcmd1 = sqlcmd1 & " c4_remail.rm_oritem = c4_or.or_oritem "
                sqlcmd1 = sqlcmd1 & "where 1=1 "
                if Request("status")="C" then	'已寄出
					if Date1<>"" then
						sqlcmd1 = sqlcmd1 & " AND (c4_remail.rm_date >= '" &Mid(Date1, 1,4)&Mid(Date1, 6,2)&Mid(Date1, 9,2)& "')"
						sqlcmd1 = sqlcmd1 & " AND (c4_remail.rm_date <= '" &Mid(Date1, 12,4)&Mid(Date1, 17,2)&Mid(Date1, 20,2)& "')"
					end if
					sqlcmd1 = sqlcmd1 & " AND c4_remail.rm_fgsent='C'"
				...
				'------- 開始輸出結果 ---				
					...
					for i = 0 to rescount-1
						...
						A1(i,0) = i + 1
						A1(i,1) = Rs1("rm_contno").Value
						A1(i,2) = Rs1("cont_sdate").Value & "~" & Rs1("cont_sdate").Value
						A1(i,3) = Rs1("or_nm").Value
						A1(i,4) = Rs1("or_addr").Value
						
						IF TRIM(Rs1("rm_date").Value) = "" THEN
							A1(i, 5) = ""
						ELSE
							A1(i,5) = Mid(Rs1("rm_date").Value, 1, 4) & "/" & Mid(Rs1("rm_date").Value, 5, 2) & "/" & Mid(Rs1("rm_date").Value, 7, 2)
						END IF
						A1(i,6) = Rs1("rm_cont").Value
						if Rs1("rm_fgsent").Value="Y" then
							A1(i,7) = "可寄出"
						else if Rs1("rm_fgsent").Value="N" then
							A1(i,7) = "目前暫時無法寄出"
						else if Rs1("rm_fgsent").Value="C" then
							A1(i,7) = "已寄出"
						else if Rs1("rm_fgsent").Value="D" then
							A1(i,7) = "不處理"
						end if
						A1(i,8) = Mid(Rs1("cont_signdate").Value, 1, 4) & "/" & Mid(Rs1("cont_signdate").Value, 5, 2) & "/" & Mid(Rs1("cont_signdate").Value, 7, 2)
						
						Dim highlight1, highlight2
						highlight1 = "A" & (5+i) & ":C" & (5+i)
						highlight2 = "D" & (5+i) & ":I" & (5+i)
						if (i mod 2) = 0
...
'					Loop
					...
					SrcBook = Server.MapPath("RptRemailList.xls")
					
					' Generate SpreadSheet and Stream to Client, Open in Place
					XLS.Generate(SrcBook, "RptRemailList.xls", True)

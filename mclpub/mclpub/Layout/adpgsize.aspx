<%@ Page Title="" Language="C#" MasterPageFile="~/MasterPage.Master" AutoEventWireup="true" CodeBehind="adpgsize.aspx.cs" Inherits="mclpub.Layout.adpgsize" %>
...
<asp:Content ID="Content1" ContentPlaceHolderID="head" runat="server">
    <script>
        ...
        function submitBtn() {
            var pgs_pgscd = $('#<% =pgs_pgscd.ClientID%>');
            var pgs_nm = $('#<% =pgs_nm.ClientID%>');
            if (pgs_pgscd.val() == "") {
                alert("廣告篇幅代碼為必填欄位!");
                return;
            }
            ...
            $.post("adpgsizeJudge.aspx", { pgs_pgscd: pgs_pgscd.val(), pgs_nm: pgs_nm.val() }, function (result) {
                if (result == "No!") {
                    alert('此筆資料已經存在!');
                }
                if (result == "Ok!") {
                    alert('新增成功!');
                    window.location.href = "adpgsize.aspx";
                }
                if (result == "Error!") {
                    alert('資料錯誤!');
                }
...
<asp:Content ID="Content2" ContentPlaceHolderID="ContentPlaceHolder1" runat="server">
    <span class="font_size12 font_darkblue font_bold">目前位置:&nbsp;落版管理 / 平面廣告落版處理 / 相關維護區
        / 廣告篇幅</span> <span class="stripeMe"></span><span class="font_size13 font_bold font_gray">
            <ol>
                <span class="font_red">註：為避免舊資料讀取錯誤，若該筆已被使用，將不允許修改/刪除！ </span>
                <br />
            </ol>
        </span>
    ...
    <span class="stripeMe">
        <asp:GridView ID="GridView1" runat="server" AutoGenerateColumns="false" PageSize="10"
            ...
            <Columns>
                <asp:BoundField DataField="pgs_pgsid" HeaderText="ID" />
                <asp:TemplateField HeaderText="廣告篇幅代碼">
                    <ItemTemplate>
                        <asp:Label ID="Label1" runat="server" Text='<%# Bind("pgs_pgscd") %>'></asp:Label>
                        <asp:TextBox ID="TextBox1" runat="server" Visible="false" Text='<%# Bind("pgs_pgscd") %>'
                            onkeyup="return ValidateNumber($(this),value)" MaxLength="2"></asp:TextBox>
                    </ItemTemplate>
                </asp:TemplateField>
                <asp:TemplateField HeaderText="廣告篇幅名稱">
                    <ItemTemplate>
                        <asp:Label ID="Label2" runat="server" Text='<%# Bind("pgs_nm") %>'></asp:Label>
                        <asp:TextBox ID="TextBox2" runat="server" Text='<%# Bind("pgs_nm") %>' Visible="false"
    ...
    <div id="dialog" style="text-align: left; display: none;" title="新增廣告篇幅">
        <span class="stripeMe">
            <table border="0" width="100%" cellspacing="0" cellpadding="0">
                <tr>
                    ...
                    <td>
                        <asp:TextBox ID="pgs_pgscd" runat="server" MaxLength="2" Width="25px" onkeyup="return ValidateNumber($(this),value)"></asp:TextBox>

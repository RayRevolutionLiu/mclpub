...
namespace mclpub
{
    public class PlaneCont2Edit_DB
    {
        private string Conn = ConfigurationManager.AppSettings["itridpr_mrlpub"];

        public DataTable SelectCust(string cust_custno)
        {
        ...
        public DataTable SelectBook()
        {
            ...
            sb.Append(@", dbo.proj.proj_bkcd, dbo.proj.proj_fgitri, dbo.proj.proj_projno, dbo.proj.proj_costctr ");
            sb.Append(@"FROM dbo.book INNER JOIN dbo.proj ON dbo.book.bk_bkcd COLLATE Chinese_Taiwan_Stroke_BIN = dbo.proj.proj_bkcd ");
            sb.Append(@"WHERE (dbo.proj.proj_syscd = 'C2') AND (dbo.proj.proj_fgitri = ' ') ");
            oCmd.CommandText = sb.ToString();
            oCmd.CommandType = CommandType.Text;
            SqlDataAdapter oda = new SqlDataAdapter(oCmd);
            DataTable ds = new DataTable();
        ...
        public DataTable SelectC2_cont()
        {
            SqlCommand oCmd = new SqlCommand();
            oCmd.Connection = new SqlConnection(Conn);
            StringBuilder sb = new StringBuilder();
            sb.Append(@"SELECT cont_contid, cont_syscd, cont_contno, cont_conttp, cont_bkcd, cont_signdate, cont_empno, cont_mfrno, cont_aunm, cont_autel");
            sb.Append(@", cont_aufax, cont_aucell, cont_auemail, cont_sdate, cont_edate, cont_totjtm, cont_madejtm, cont_restjtm");
            sb.Append(@", cont_disc, cont_freetm, cont_fgclosed, cont_tottm, cont_pubtm, cont_resttm, cont_chgjtm, cont_custno");
            sb.Append(@", cont_totamt, cont_pubamt, cont_chgamt, cont_paidamt, cont_restamt, cont_clrtm, cont_menotm, cont_getclrtm");
            sb.Append(@", cont_oldcontno, cont_moddate, cont_fgpayonce, cont_modempno, cont_fgcancel, cont_credate, cont_adamt");
            sb.Append(@", cont_freebkcnt, cont_remark, cont_fgtemp, cont_fgpubed, cont_restclrtm, cont_restmenotm, cont_restgetclrtm, cont_njtpcnt FROM dbo.c2_cont");
            oCmd.CommandText = sb.ToString();
            oCmd.CommandType = CommandType.Text;
            SqlDataAdapter oda = new SqlDataAdapter(oCmd);
            DataTable ds = new DataTable();
        ...
        public void UpdateC2_cont(string strSyscd, string strContNo, string strContType, string strBkcd, string strSignDate, string strEmpNo, string strStartDate, string strEndDate,
            string strfgClosed, string strModifyDate, string strfgPayOnce, string strModifyEmpNo, string strfgCancel, string strTotalJTime, string strMadeJTime, string strRestJTime, string strTotalTime,
            string strPubTime, string strRestTime, string strTotalAmount, string strPaidAmount, string strRestAmount, string strChangeJTime, string strFreeTime, string strADAmount,
            string strDiscount, string strPubADAmount, string strPubChangeAmount, string strColorTime, string strGetColorTime, string strMemoTime, string strFreeBookCount,string strRemark,
            string strAUName, string strAUTel, string strAUFax, string strAUCell, string strAUEmail)
        {
            SqlCommand oCmd = new SqlCommand();
            oCmd.Connection = new SqlConnection(Conn);
            StringBuilder sb = new StringBuilder();
            sb.Append(@"UPDATE c2_cont SET");
            sb.Append(@" cont_conttp = @conttp, cont_bkcd = @bkcd, cont_signdate = @signdate, cont_empno = @empno, cont_aunm = @aunm, cont_autel = @autel, cont_aufax = @aufax,");
            sb.Append(@" cont_aucell = @aucell, cont_auemail = @auemail, cont_sdate = @sdate, cont_edate = @edate, cont_totjtm = @totjtm, cont_madejtm = @madejtm,");
            sb.Append(@" cont_restjtm = @restjtm, cont_disc = @disc, cont_freetm = @freetm, cont_fgclosed = @fgclosed, cont_tottm = @tottm, cont_pubtm = @pubtm, cont_resttm = @resttm,");
            sb.Append(@" cont_chgjtm = @chgjtm, cont_totamt = @totamt, cont_pubamt = @pubamt, cont_chgamt = @chgamt, cont_paidamt = @paidamt, cont_restamt = @restamt,");
            sb.Append(@" cont_clrtm = @clrtm, cont_menotm = @menotm, cont_getclrtm = @getclrtm, cont_moddate = @moddate, cont_fgpayonce = @fgpayonce, cont_modempno = @modempno,");
            sb.Append(@" cont_fgcancel = @fgcancel, cont_adamt = @adamt, cont_freebkcnt = @freebkcnt, cont_remark = @remark WHERE (cont_syscd = @syscd) AND (cont_contno = @contno)");
            oCmd.CommandText = sb.ToString();
            oCmd.CommandType = CommandType.Text;
            SqlDataAdapter oda = new SqlDataAdapter(oCmd);
            // 合約書基本資料 區
            ...
            oCmd.Parameters.AddWithValue("@menotm", strMemoTime);

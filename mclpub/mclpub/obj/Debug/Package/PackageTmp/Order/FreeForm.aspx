						...
						<td>
							<TABLE dataFld="明細表" id="tbItems" dataSrc="#DSO0" cellSpacing="0" cellPadding="0" border="0" width="100%" class="font_blacklink font_size13">
								...
								<tr>
									...
									<td>
										<INPUT dataFld="計劃代號" id="tbxProj" style="WIDTH: 90px; HEIGHT: 22px" readOnly type="text" maxLength="10" size="7" name="tt3">
        ...
        $("#<% =tbxOrderDate.ClientID%>").datepicker(
{
    ...
    currentText: "今天"
		...
		<script language="javascript">
		    ...
		    xmlReceivers.childNodes.item(0).selectSingleNode("地址").text = document.getElementById("<% =hiddenCoAddress.ClientID%>").value;
		...
		<script language="javascript">
		    ...
		    function doSelectBook(obj) {
		        ...
		        for (i = 0; i < ItemBook.childNodes.length; i++) {
		            var one = ItemBook.childNodes.item(i).selectSingleNode("obtp_otp1cd").text;
		            var two = document.all("<% =hiddenType1.ClientID%>").value;
		            var three = ItemBook.childNodes.item(i).selectSingleNode("fgitri").text;
		            //因為ItemBook.childNodes.item(i).selectSingleNode("fgitri").text=" "(有空格) 要用JA的REPLACE TRIM掉" "
		    ...
		    function pick_dateStart(obj) {
		        ...
		        if (vreturn) {
		            ...
		            if (window.document.all("<% =hiddenType1.ClientID%>").value == "02") {
		                ...
		                xmlItems.childNodes.item(idx).selectSingleNode("訂閱訖時").text = "9999/12/31";
		        }
		        return true;
		    }

<%@ Page Title="" Language="C#" MasterPageFile="~/MasterPage.Master" AutoEventWireup="true" CodeBehind="adcolor.aspx.cs" Inherits="mclpub.Layout.adcolor" %>
...
<asp:Content ID="Content1" ContentPlaceHolderID="head" runat="server">
    <script>
        ...
        function submitBtn() {
            var clr_clrcd = $('#<% =clr_clrcd.ClientID%>');
            var clr_nm = $('#<% =clr_nm.ClientID%>');
            if (clr_clrcd.val() == "") {
                alert("廣告色彩代碼為必填欄位!");
                return;
            }
            ...
            $.post("adcolorJudge.aspx", { clr_clrcd: clr_clrcd.val(), clr_nm: clr_nm.val() }, function (result) {
                if (result == "No!") {
                    alert('此筆資料已經存在!');
                }
                if (result == "Ok!") {
                    alert('新增成功!');
                    window.location.href = "adcolor.aspx";
                }
                if (result == "Error!") {
                    alert('資料錯誤!');
                }
...
<asp:Content ID="Content2" ContentPlaceHolderID="ContentPlaceHolder1" runat="server">
    <span class="font_size12 font_darkblue font_bold">目前位置:&nbsp;落版管理 / 平面廣告落版處理 / 相關維護區
        / 廣告色彩</span> <span class="stripeMe"></span><span class="font_size13 font_bold font_gray">
            <ol>
    ...
    <span class="stripeMe">
        <asp:GridView ID="GridView1" runat="server" AutoGenerateColumns="false" PageSize="10"
            ...
            <Columns>
                <asp:BoundField DataField="clr_clrid" HeaderText="ID" />
                <asp:TemplateField HeaderText="廣告色彩代碼">
                    <ItemTemplate>
                        <asp:Label ID="Label1" runat="server" Text='<%# Bind("clr_clrcd") %>'></asp:Label>
                        <asp:TextBox ID="TextBox1" runat="server" Visible="false" Text='<%# Bind("clr_clrcd") %>'
                            onkeyup="return ValidateNumber($(this),value)" MaxLength="2"></asp:TextBox>
                    </ItemTemplate>
                </asp:TemplateField>
                <asp:TemplateField HeaderText="廣告色彩名稱">
                    <ItemTemplate>
                        <asp:Label ID="Label2" runat="server" Text='<%# Bind("clr_nm") %>'></asp:Label>
                        <asp:TextBox ID="TextBox2" runat="server" Text='<%# Bind("clr_nm") %>' Visible="false"
    ...
    <div id="dialog" style="text-align: left; display: none;" title="新增廣告色彩">
        <span class="stripeMe">
            <table border="0" width="100%" cellspacing="0" cellpadding="0">
                <tr>
                    ...
                    <td>
                        <asp:TextBox ID="clr_clrcd" runat="server" MaxLength="2" Width="25px" onkeyup="return ValidateNumber($(this),value)"></asp:TextBox>

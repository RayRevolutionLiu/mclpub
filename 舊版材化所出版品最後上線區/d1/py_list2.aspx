	...
	<body>
		<form id="odlist" method="post" runat="server">
			<%
				...
				Dim EmpCName
				Dim	preNo, preIano, prePyno, count
				' Open Database------------------
				...
				sqlcmd1 = sqlcmd1 & "WHERE (py.py_pytpcd = '02') and py_pysdate='"& Request("yyyymm") & "' and py_pysseq='"&Request("batch") &"'"
...
'								A1(i,3) = Rs1("ia_pyat").Value
								...
								A1(i,10) = Rs1("ia_invno").Value
								A1(i,13) = Rs1("ia_syscd").Value
								A1(i,14) = Rs1("ia_iano").Value
							end if
						else
							A1(i,0) = count + 1
							count = count + 1
							A1(i,1) = Rs1("iad_syscd").Value & Trim(Rs1("iad_fk1").Value) & Trim(Rs1("iad_fk2").Value) & Trim(Rs1("iad_fk3").Value)
							if Rs1("py_pyno").Value = prePyno then
								A1(i,2) = ""
								A1(i,3) = ""
							else
								A1(i,2) = Rs1("py_pyno").Value
								A1(i,3) = Rs1("py_amt").Value
							end if
'							A1(i,3) = Rs1("ia_pyat").Value
							A1(i,4) = Rs1("ia_rnm").Value
							A1(i,5) = Rs1("mfr_inm").Value
							A1(i,6) = Rs1("ia_mfrno").Value
							A1(i,7) = Rs1("py_chkno").Value
							A1(i,8) = Rs1("py_chkbnm").Value
							A1(i,9) = Mid(Rs1("py_chkdate").Value, 1, 4) & "/" & Mid(Rs1("py_chkdate").Value, 5, 2) & "/" & Mid(Rs1("py_chkdate").Value, 7, 2)
							A1(i,10) = Rs1("ia_invno").Value
							A1(i,13) = Rs1("ia_syscd").Value
							A1(i,14) = Rs1("ia_iano").Value
						end if
						A1(i,11) = Rs1("iad_projno").Value
						A1(i,12) = Rs1("iad_desc").Value
						
						preNo = Rs1("iad_syscd").Value & Trim(Rs1("iad_fk1").Value) & Trim(Rs1("iad_fk2").Value) & Trim(Rs1("iad_fk3").Value)
						preIano = Rs1("ia_iano").Value
						prePyno = Rs1("py_pyno").Value
...
'					Loop
					...
					SrcBook = Server.MapPath("py_list2.xls")
					
					' Generate SpreadSheet and Stream to Client, Open in Place
					XLS.Generate(SrcBook, "py_list2.xls", True)

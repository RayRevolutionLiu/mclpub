...
namespace mclpub.Contract
{
    public partial class ContShowDetail : System.Web.UI.Page
    {
        ...
        private void GetMfrCust()
        {
            ...
            string StrCust = sec.decryptquerystring(Request.QueryString["cust_custno"].ToString().Trim());
            // 若有客戶編號, 則指定顯示之
            if (StrCust != "")
            {
                custno = StrCust;
        ...
        {
            // 抓出網頁變數: 客戶編號 
            string strOldContNo = sec.decryptquerystring(Context.Request.QueryString["cont_contno"].ToString().Trim());

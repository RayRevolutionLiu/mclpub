			...
			if (!IsPostBack)
			{
				...
				else
				{
					...
					for(i=0; i< dgdCont.Items.Count ; i++)
					{
						...
						// 已註銷註記
						string strfgcancel = dgdCont.Items[i].Cells[16].Text.ToString().Trim();
						//Response.Write("strfgcancel= " + strfgcancel + "<br>");
						if(strfgcancel == "0")
						{
							dgdCont.Items[i].Cells[16].Text = "否";
						}
						else
						{
							dgdCont.Items[i].Cells[16].Text = "<font color=red>是</font>";
						}
...
//				"nfo=True;user id=sa;workstation id=03212-890711;packet size=4096";
			...
			// sqlSelectCommand1
			// 
			this.sqlSelectCommand1.CommandText = @"SELECT c2_cont.cont_contid, c2_cont.cont_syscd, c2_cont.cont_contno, c2_cont.cont_custno, c2_cont.cont_conttp, c2_cont.cont_bkcd, c2_cont.cont_signdate, c2_cont.cont_mfrno, c2_cont.cont_aunm, c2_cont.cont_autel, c2_cont.cont_fgclosed, c2_cont.cont_fgpayonce, book.bk_nm, book.bk_bkcd, mfr.mfr_inm, c2_cont.cont_disc, mfr.mfr_mfrno, c2_cont.cont_clrtm, c2_cont.cont_menotm, c2_cont.cont_getclrtm, c2_cont.cont_fgpubed, c2_cont.cont_sdate, c2_cont.cont_edate, c2_cont.cont_fgcancel, c2_cont.cont_fgtemp FROM c2_cont INNER JOIN book ON c2_cont.cont_bkcd = book.bk_bkcd INNER JOIN mfr ON c2_cont.cont_mfrno = mfr.mfr_mfrno ORDER BY c2_cont.cont_contno DESC";
		...
		private void dgdCont_ItemCommand(object source, System.Web.UI.WebControls.DataGridCommandEventArgs e)
		{
			...
			// 若按下確定修改的動作, 做轉址到 ContFm_modify.aspx
			else if(e.CommandName=="OK")
			{
				...
				{
					string fgclosed = dgdCont.SelectedItem.Cells[10].Text.Trim();
					...
					{
						lblCaption.Text = "";
						Response.Redirect("ContFm_modify.aspx?custno=" + Context.Request.QueryString["custno"] + "&contno=" + dgdCont.SelectedItem.Cells[0].Text.Trim());
						//if(Context.Request.QueryString["conttp"]=="01")
							//Response.Redirect("ContFm_modify.aspx?custno=" + Context.Request.QueryString["custno"] + "&contno=" + dgdCont.SelectedItem.Cells[0].Text.Trim());
						//else if(Context.Request.QueryString["conttp"]=="09")
							//Response.Redirect("ContFm_modify.aspx?custno=" + Context.Request.QueryString["custno"] + "&contno=" + dgdCont.SelectedItem.Cells[0].Text.Trim());

...
namespace mclpub.Contract
{
    public partial class ORForm : System.Web.UI.Page
    {
        ...
        private void ModifyDB()
        {
            ...
            int strORPubCount = 0;
            int strORUnPubCount = 0;
            try
            {
                strORPubCount = Convert.ToInt32(tbxORPubCount.Text.ToString().Trim() != "" ? tbxORPubCount.Text.ToString().Trim() : "0");
                strORUnPubCount = Convert.ToInt32(tbxORUnPubCount.Text.ToString().Trim() != "" ? tbxORUnPubCount.Text.ToString().Trim() : "0");
            }
            catch (Exception ex)
            {
                JavaScript.AlertMessage(this.Page, "登本數請輸入數字");
                return;
            }

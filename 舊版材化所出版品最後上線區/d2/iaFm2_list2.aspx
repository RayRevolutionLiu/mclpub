	...
	<body>
		<form id="cont_list2" method="post" runat="server">
			<%
				...
				' 自訂 sql 變數
				Dim strYYYYMM, strBkcd, strEmpNo, strSort, strIabSeq
				...
				if(Request("yyyymm") <> "") then
					strBkcd = Request("bkcd")
					sqlcmd2 = sqlcmd2 & " AND (ia_iabdate = '" & strYYYYMM & "') "
				else
					sqlcmd2 = sqlcmd2
				end if
				...
				sqlcmd1 = sqlcmd1 & " AS iad_fk1, Substring(RTRIM(iad.iad_fk2), 1, 4) "
				...
				sqlcmd1 = sqlcmd1 & " iad.iad_amt, RTRIM(ia.ia_contno) AS ia_contno, ia.ia_iabdate, "
				sqlcmd1 = sqlcmd1 & " ia.ia_iabseq, "
				...
				if(Request("yyyymm") <> "") then
					strBkcd = Request("bkcd")
					sqlcmd1 = sqlcmd1 & " AND (ia_iabdate = '" & strYYYYMM & "') "
				else
					sqlcmd1 = sqlcmd1
				end if
				...
				' 若有資料, 則輸出至 ExcelSpeedGen
					...
					SrcBook = Server.MapPath("iaFm2_list2.xls")

					' Generate SpreadSheet and Stream to Client, Open in Place
					XLS.Generate(SrcBook, "iaFm2_list2.xls", True)

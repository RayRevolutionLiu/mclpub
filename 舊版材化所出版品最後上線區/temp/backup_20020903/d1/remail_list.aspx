	...
	<body>
		<form id="odlist" method="post" runat="server">
			<%
				...
				sqlcmd1 = "SELECT  *, c1_or.or_nm, c1_or.or_addr, rm_syscd+rm_custno+rm_otp1cd+rm_otp1seq AS nostr,c1_order.o_date "
				sqlcmd1 = sqlcmd1 & " FROM c1_remail INNER JOIN "
                sqlcmd1 = sqlcmd1 & "c1_or ON c1_remail.rm_syscd = c1_or.or_syscd AND "
                sqlcmd1 = sqlcmd1 & "c1_remail.rm_custno = c1_or.or_custno AND "
				sqlcmd1 = sqlcmd1 & "c1_remail.rm_otp1cd = c1_or.or_otp1cd AND "
                sqlcmd1 = sqlcmd1 & "c1_remail.rm_otp1seq = c1_or.or_otp1seq AND "
                sqlcmd1 = sqlcmd1 & "c1_remail.rm_oritem = c1_or.or_oritem INNER JOIN "
                ...
                if Request("status")="C" then	'已寄出
					if Date1<>"" then
						sqlcmd1 = sqlcmd1 & " AND (c1_remail.rm_date >= '" &Mid(Date1, 1,4)&Mid(Date1, 6,2)&Mid(Date1, 9,2)& "')"
						sqlcmd1 = sqlcmd1 & " AND (c1_remail.rm_date <= '" &Mid(Date1, 12,4)&Mid(Date1, 17,2)&Mid(Date1, 20,2)& "')"
					end if
					sqlcmd1 = sqlcmd1 & " AND c1_remail.rm_fgsent='C'"
				...
				'------- 開始輸出結果 ---				
					...
					for i = 0 to rescount-1
						...
						A1(i,2) = Rs1("rm_sdate").Value & "~" & Rs1("rm_edate").Value
						...
						A1(i,8) = Mid(Rs1("o_date").Value, 1, 4) & "/" & Mid(Rs1("o_date").Value, 5, 2) & "/" & Mid(Rs1("o_date").Value, 7, 2)
...
'					Loop
					...
					SrcBook = Server.MapPath("remail_list.xls")
					
					' Generate SpreadSheet and Stream to Client, Open in Place
					XLS.Generate(SrcBook, "remail_list.xls", True)

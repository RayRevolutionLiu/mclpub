...
{
	/// <summary>
	/// Summary description for ORCounts_stat_search1.
	/// </summary>
	public class ORCounts_stat_search1 : System.Web.UI.Page
	{
		...
		private void CountData()
		{
			...
			{
				rowfilterstr4 = rowfilterstr4 + " AND cont_conttp='" + strConttp + "'";
			}
			else
			{
				rowfilterstr4 = rowfilterstr4;
			}

			// 郵寄地區
			if(this.cbx1.Checked)
			{
				rowfilterstr4 = rowfilterstr4 + " AND or_fgmosea='" + fgMOSea + "'";
			}
			else
			{
				rowfilterstr4 = rowfilterstr4;
			...
			{
				rowfilterstr4 = rowfilterstr4 + " AND or_mtpcd='" + Mtpcd + "'";
			}
			else
			{
				rowfilterstr4 = rowfilterstr4;
			...
			if(dv4.Count > 0)
			{
				// 轉址
				// 書籍類別 & 刊登年月 篩選條件
				string RedirectURL = "ORCounts_stat2a.aspx?bkcd=" + strBkcd + "&yyyymm=" + strYYYYMM;
				// 合約類別 篩選條件
				if(this.cbx0.Checked)
				{
				...
				this.lblMessage.Text="結果: 找到 <B>" + dv4.Count + "</B>筆資料; 請繼續按 <A HREF='" + RedirectURL + "' Target='_blank'>此連結</A> 來繼續進行下一動作!<br>";
			}
			else
			{
		...
		{
			Response.Redirect("ORCounts_stat_search1.aspx");
		}
...
//				"rue;user id=webuser;workstation id=17-0890711-01;packet size=4096";
			...
			// sqlSelectCommand4
			//
			this.sqlSelectCommand4.CommandText = @"SELECT DISTINCT dbo.c2_or.or_contno, dbo.c2_cont.cont_conttp, dbo.c2_cont.cont_bkcd, RTRIM(dbo.book.bk_nm) AS bk_nm, RTRIM(dbo.c2_cont.cont_mfrno) AS cont_mfrno, RTRIM(dbo.mfr.mfr_inm) AS mfr_inm, dbo.c2_or.or_fgmosea, dbo.c2_or.or_mtpcd, RTRIM(dbo.mtp.mtp_nm) AS mtp_nm, COUNT(dbo.c2_or.or_pubcnt) AS PubCounts, dbo.c2_pub.pub_yyyymm, dbo.c2_cont.cont_sdate, dbo.c2_cont.cont_edate, SUBSTRING(dbo.c2_cont.cont_sdate, 1, 4) + '/' + SUBSTRING(dbo.c2_cont.cont_sdate, 5, 2) + ' ~ ' + SUBSTRING(dbo.c2_cont.cont_edate, 1, 4) + '/' + SUBSTRING(dbo.c2_cont.cont_edate, 5, 2) AS cont_sedate FROM dbo.c2_cont INNER JOIN dbo.c2_or ON dbo.c2_cont.cont_syscd = dbo.c2_or.or_syscd AND dbo.c2_cont.cont_contno = dbo.c2_or.or_contno INNER JOIN dbo.mfr ON dbo.c2_cont.cont_mfrno = dbo.mfr.mfr_mfrno INNER JOIN dbo.mtp ON dbo.c2_or.or_mtpcd = dbo.mtp.mtp_mtpcd INNER JOIN dbo.c2_pub ON dbo.c2_cont.cont_syscd = dbo.c2_pub.pub_syscd AND dbo.c2_cont.cont_contno = dbo.c2_pub.pub_contno INNER JOIN dbo.book ON dbo.c2_cont.cont_bkcd = dbo.book.bk_bkcd WHERE (dbo.c2_cont.cont_fgclosed = '0') AND (dbo.c2_cont.cont_fgcancel = '0') AND (dbo.c2_cont.cont_fgtemp = '0') GROUP BY dbo.c2_or.or_contno, dbo.c2_cont.cont_conttp, dbo.c2_cont.cont_bkcd, dbo.book.bk_nm, dbo.c2_cont.cont_mfrno, dbo.mfr.mfr_inm, dbo.c2_or.or_fgmosea, dbo.c2_or.or_mtpcd, dbo.mtp.mtp_nm, dbo.c2_pub.pub_yyyymm, dbo.c2_cont.cont_sdate, dbo.c2_cont.cont_edate ORDER BY dbo.c2_or.or_mtpcd, dbo.c2_or.or_contno";

...
'					oConn.Open("Provider=SQLOLEDB.1;Data Source=140.96.254.61,6161;User ID=pubmrlpub;Password=p%#ass;Initial Catalog=mrlpub")
				...
				title=Mid(date_str, 1, 4)&"年"&Mid(date_str, 5, 2)&"月"&Request("bookno")&"期  "&Request("btpcd")&" 印製份數統計表"
				...
				sqlcmd1 = "SELECT dbo.tmp_statistics.*, dbo.c1_otp.otp_otp1nm, dbo.c1_otp.otp_otp2nm, dbo.mtp.mtp_nm "
				sqlcmd1 = sqlcmd1 & "FROM dbo.tmp_statistics LEFT OUTER JOIN dbo.c1_otp ON "
                ...
                sqlcmd1 = sqlcmd1 & "INNER JOIN dbo.mtp ON  dbo.tmp_statistics.tmp_btpcd COLLATE Chinese_Taiwan_Stroke_CI_AS = dbo.mtp.mtp_mtpcd "
   				sqlcmd1 = sqlcmd1 & "WHERE (dbo.tmp_statistics.tmp_param2 = 1)"
				sqlcmd1 = sqlcmd1 & "ORDER BY tmp_btpcd, tmp_otp1cd, tmp_otp2cd"
				
				sqlcmd2 = "SELECT dbo.tmp_statistics.*, dbo.c1_otp.otp_otp1nm, dbo.c1_otp.otp_otp2nm, dbo.mtp.mtp_nm "
				sqlcmd2 = sqlcmd2 & "FROM dbo.tmp_statistics LEFT OUTER JOIN dbo.c1_otp ON "
                ...
                sqlcmd2 = sqlcmd2 & "INNER JOIN dbo.mtp ON  dbo.tmp_statistics.tmp_btpcd COLLATE Chinese_Taiwan_Stroke_CI_AS = dbo.mtp.mtp_mtpcd "
				sqlcmd2 = sqlcmd2 & "WHERE (dbo.tmp_statistics.tmp_param2 = 2) "
				sqlcmd2 = sqlcmd2 & "ORDER BY tmp_btpcd , tmp_otp1cd, tmp_otp2cd"

				sqlcmd3 = "SELECT dbo.tmp_statistics.*, dbo.c1_otp.otp_otp1nm, dbo.c1_otp.otp_otp2nm, dbo.mtp.mtp_nm "
				sqlcmd3 = sqlcmd3 & "FROM dbo.tmp_statistics LEFT OUTER JOIN dbo.c1_otp ON "
                ...
                sqlcmd3 = sqlcmd3 & "INNER JOIN dbo.mtp ON  dbo.tmp_statistics.tmp_btpcd COLLATE Chinese_Taiwan_Stroke_CI_AS = dbo.mtp.mtp_mtpcd "
				sqlcmd3 = sqlcmd3 & "WHERE (dbo.tmp_statistics.tmp_param2 = 3) "
				sqlcmd3 = sqlcmd3 & "ORDER BY tmp_btpcd,tmp_otp1cd, tmp_otp2cd"

				sqlcmd4 = "SELECT dbo.tmp_statistics.*, dbo.c1_otp.otp_otp1nm, dbo.c1_otp.otp_otp2nm, dbo.mtp.mtp_nm "
				sqlcmd4 = sqlcmd4 & "FROM dbo.tmp_statistics LEFT OUTER JOIN dbo.c1_otp ON "
                ...
                sqlcmd4 = sqlcmd4 & "INNER JOIN dbo.mtp ON  dbo.tmp_statistics.tmp_btpcd COLLATE Chinese_Taiwan_Stroke_CI_AS = dbo.mtp.mtp_mtpcd "
				sqlcmd4 = sqlcmd4 & "WHERE (dbo.tmp_statistics.tmp_param2 = 4) "
				sqlcmd4 = sqlcmd4 & "ORDER BY tmp_btpcd,tmp_otp1cd, tmp_otp2cd"

				sqlcmd5 = "SELECT dbo.tmp_statistics.*, dbo.c1_otp.otp_otp1nm, dbo.c1_otp.otp_otp2nm, dbo.mtp.mtp_nm "
				sqlcmd5 = sqlcmd5 & "FROM dbo.tmp_statistics LEFT OUTER JOIN dbo.c1_otp ON "
                ...
                sqlcmd5 = sqlcmd5 & "INNER JOIN dbo.mtp ON  dbo.tmp_statistics.tmp_btpcd COLLATE Chinese_Taiwan_Stroke_CI_AS = dbo.mtp.mtp_mtpcd "
				sqlcmd5 = sqlcmd5 & "WHERE (dbo.tmp_statistics.tmp_param2 = 5) "
				sqlcmd5 = sqlcmd5 & "ORDER BY tmp_btpcd,tmp_otp1cd, tmp_otp2cd"
				...
				sqlcmd6 = sqlcmd6 & "INNER JOIN dbo.mtp ON dbo.c1_ramt.ra_mtpcd = dbo.mtp.mtp_mtpcd "
				...
				if Rs1.EOF then
					...
					for i = 0 to rescount - 1 step 1
						' A 欄 = A1(i,0)
						if Rs1("tmp_btpcd").Value = premtp then
							A1(i,0) = ""
							if Rs1("tmp_otp1cd").Value = preotp1cd then
								A1(i,1) = ""
							else
								A1(i,1) = Rs1("otp_otp1nm").Value
							end if
						else
							A1(i,0) = Rs1("mtp_nm").Value
							A1(i,1) = Rs1("otp_otp1nm").Value
						end if
						A1(i,2) = Rs1("otp_otp2nm").Value
						A1(i,3) = Rs1("tmp_param1").Value
						
						preotp1cd = Rs1("tmp_otp1cd").Value
						premtp = Rs1("tmp_btpcd").Value
					...
					for i = 0 to rescount - 1 step 1
						' A 欄 = A1(i,0)
						if Rs2("tmp_btpcd").Value = premtp then
							A2(i,0) = ""
							if Rs2("tmp_otp1cd").Value = preotp1cd then
								A2(i,1) = ""
							else
								A2(i,1) = Rs2("otp_otp1nm").Value
							end if
						else
							A2(i,0) = Rs2("mtp_nm").Value
							A2(i,1) = Rs2("otp_otp1nm").Value
						end if
						A2(i,2) = Rs2("otp_otp2nm").Value
						A2(i,3) = Rs2("tmp_param1").Value
						
						preotp1cd = Rs2("tmp_otp1cd").Value
						premtp = Rs2("tmp_btpcd").Value
					...
					for i = 0 to rescount - 1 step 1
						' A 欄 = A1(i,0)
						if Rs3("tmp_btpcd").Value = premtp then
							A3(i,0) = ""
							if Rs3("tmp_otp1cd").Value = preotp1cd then
								A3(i,1) = ""
							else
								A3(i,1) = Rs3("otp_otp1nm").Value
							end if
						else
							A3(i,0) = Rs3("mtp_nm").Value
							A3(i,1) = Rs3("otp_otp1nm").Value
						end if
						A3(i,2) = Rs3("otp_otp2nm").Value
						A3(i,3) = Rs3("tmp_param1").Value
						
						preotp1cd = Rs3("tmp_otp1cd").Value
						premtp = Rs3("tmp_btpcd").Value
					...
					for i = 0 to rescount - 1 step 1
						' A 欄 = A4(i,0)
						if Rs4("tmp_btpcd").Value = premtp then
							A4(i,0) = ""
							if Rs4("tmp_otp1cd").Value = preotp1cd then
								A4(i,1) = ""
							else
								A4(i,1) = Rs4("otp_otp1nm").Value
							end if
						else
							A4(i,0) = Rs4("mtp_nm").Value
							A4(i,1) = Rs4("otp_otp1nm").Value
						end if
						A4(i,2) = Rs4("otp_otp2nm").Value
						A4(i,3) = Rs4("tmp_param1").Value
						
						preotp1cd = Rs4("tmp_otp1cd").Value
						premtp = Rs4("tmp_btpcd").Value
					...
					for i = 0 to rescount - 1 step 1
						' A 欄 = A5(i,0)
						if Rs5("tmp_btpcd").Value = premtp then
							A5(i,0) = ""
							if Rs5("tmp_otp1cd").Value = preotp1cd then
								A5(i,1) = ""
							else
								A5(i,1) = Rs5("otp_otp1nm").Value
							end if
						else
							A5(i,0) = Rs5("mtp_nm").Value
							A5(i,1) = Rs5("otp_otp1nm").Value
						end if
						A5(i,2) = Rs5("otp_otp2nm").Value
						A5(i,3) = Rs5("tmp_param1").Value
						
						preotp1cd = Rs5("tmp_otp1cd").Value
						premtp = Rs5("tmp_btpcd").Value
					...
					if Rs6.EOF=false then
						...
						' Array 6
						ReDim A6(rescount-1,3)
	
						' Populate Array 6
						for i = 0 to rescount - 1 step 1
						' A 欄 = A6(i,0)
							A6(i,0) = Rs6("mtp_nm").Value
							A6(i,1) = Rs6("ra_mnt").Value
							A6(i,2)	= 1		
							Rs6.MoveNext
					...
					SrcBook = Server.MapPath("book_mnt2.xls")
					
					' Generate SpreadSheet and Stream to Client, Open in Place
					XLS.Generate(SrcBook, "book_mnt2.xls", True)

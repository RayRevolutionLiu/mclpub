<%@ Page Title="" Language="C#" MasterPageFile="~/MasterPage.Master" AutoEventWireup="true" CodeBehind="RecoveryPayList.aspx.cs" Inherits="mclpub.Pay.RecoveryPayList" %>
...
<script>
    function DelPayList() {
        if (confirm("確定是否要刪除?"));
        {
            var pys_pysdate = $("#lblyyyymm").val();
            var pys_pysseq = $("#lblbatch").val();

            $.post("RecoveryPayListDel.ashx", { pys_pysdate: pys_pysdate, pys_pysseq: pys_pysseq }, function (result) {
                if (result == "OK") {
                    alert("刪除繳款資料已完成");
                    $("#btnPrint").attr("disabled", "disabled");
                    location.href = "RecoveryPayList.aspx";
                }
                else {
                    alert(result);
                    return;
...
<asp:Content ID="Content2" ContentPlaceHolderID="ContentPlaceHolder1" runat="server">
    <span class="font_size12 font_darkblue font_bold">目前位置:&nbsp;帳務管理 / 繳款處理 / 繳款清單回復</span> 
<span class="stripeMe">
...
<span class="stripeMe">
    ...
    <div id="dialogTable" style="text-align: left; display:none;">
        ...
        <input id="btnPrint" type="button" value="刪除繳款清單" class="btn_mouseout"
         onmouseover="this.className='btn_mouseover'" onmouseout="this.className='btn_mouseout'" 
         onclick="DelPayList();" disabled="disabled" />
